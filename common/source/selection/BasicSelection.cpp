...
#include "../../header/selection/BasicSelection.hpp"
 ...
 */
BasicSelection::BasicSelection(unsigned long id, GlobalEventStreamTypedef &globalEventStream,
 ...
 */
void BasicSelection::pushNewEvent(GlobalEventStreamTypedef_iterator it)
{
...
unsigned long BasicSelection::getId() const { return this->id; }

unsigned long BasicSelection::getStartTimestamp() const
{
...
void BasicSelection::setStartTimestamp(unsigned long startTimestamp) { this->startTimestamp = startTimestamp; }

BasicSelection::GlobalEventStreamTypedef &BasicSelection::getGlobalEventStream() { return this->globalEventStream; }

typename BasicSelection::GlobalEventStreamTypedef_iterator BasicSelection::getStartPosition() const
{
    return this->startPosition;
}

void BasicSelection::setStartPosition(GlobalEventStreamTypedef_iterator startPosition)
{
 ...
 */
typename BasicSelection::GlobalEventStreamTypedef_iterator BasicSelection::getLastPosition() const
{
 ...
 * protected by lock (mutex)
 */
void BasicSelection::setLastPosition(GlobalEventStreamTypedef_iterator lastPosition)
{
 ...
 *         else -1
 */
int BasicSelection::getEventIndex(GlobalEventStreamTypedef_iterator it)
{
    shared_lock<boost::shared_mutex> shared_lk(this->shared_mtx_item); // read lock (shared)
 ...
 */
typename BasicSelection::GlobalEventStreamTypedef_iterator BasicSelection::getEventIterator(int index)
{
    shared_lock<boost::shared_mutex> shared_lk(this->shared_mtx_item); // read lock (shared)
...
typename BasicSelection::GlobalEventStreamTypedef_iterator BasicSelection::getEndPosition() const
{
    shared_lock<boost::shared_mutex> shared_lk(this->shared_mtx_item); // read lock (shared)
    return this->endPosition;
}

int BasicSelection::getSize() const
{
    shared_lock<boost::shared_mutex> shared_lk(this->shared_mtx_item); // read lock (shared)
    return this->size;
}
 ...
 */
bool BasicSelection::isSelectionClosed() const
{
    shared_lock<boost::shared_mutex> shared_lk(this->shared_mtx_item); // read lock (shared)
 ...
 * 				else false
 */
bool BasicSelection::hasEqualOrHigherPriority(unsigned long Id) const
{
 ...
 * 				else false
 */
bool BasicSelection::hasHigherPriority(unsigned long Id) const
{
    // I didn't consider the rotation of selection Id
    // there is no selection with Id=0
    if ((Id == 0) || (this->id < Id))
...
bool BasicSelection::isReadyToBeRemoved() const
{
    lock_guard<mutex> lk(mtx_remove);
    return this->readyToBeRemoved;
}

void BasicSelection::setReadyToBeRemoved(bool readyToBeRemoved)
{
    lock_guard<mutex> lk(mtx_remove);
